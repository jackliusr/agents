...
os.environ["USER_AGENT"] = "MyFinancialAgentApp/1.0 (contact: your-email@example.com)"
# Configure DSPy
lm = dspy.LM(model='deepseek/deepseek-v4-flash',
             api_Key = API_KEY,
...
def get_stock_news(ticker: str) -> str:
    """Get latest news headlines for a stock ticker symbol."""
    try:
        stock = yf.Ticker(ticker)
        news = stock.news
        if not news:
            return json.dumps({"ticker": ticker, "news": [], "message": "No news found"})
        ...
        return json.dumps({"ticker": ticker, "news": articles}, indent=2)
...
class FinancialAnalysisAgent(dspy.Module):
    ...
    def __init__(self):
        ...
        # Combine all tools
        self.tools = [
            get_stock_news,  # LangChain Yahoo Finance News
            get_stock_price,
            compare_stocks
        ]
